...
function resetZoomPan(element) {
    ...
    if (element) {
     ...
     if (flipbookElement && hammerManager) {
         hammerManager.get('pan').set({ enable: true, direction: Hammer.DIRECTION_ALL });
     }
}
...
function setupHammer(element) {
    ...
    const pan = new Hammer.Pan({ pointers: 0, direction: Hammer.DIRECTION_ALL }); // Allow 1 or 2 fingers

    // Allow pinch and 2-finger pan to recognize simultaneously
    pinch.recognizeWith(pan);

    hammerManager.add([pinch, pan]);

    let initialOffsetX = currentOffsetX;
    ...
    hammerManager.on('pinchstart', (e) => {
        ...
        element.style.transition = 'none'; // Smoother immediate feedback
    });
    ...
    hammerManager.on('pinchend pinchcancel', (e) => {
        if (isDrawing) return;
        isPinching = false;
        element.style.transition = ''; // Restore transition for reset
        // Ensure pan offsets are still valid after scaling
        adjustPanLimits(element);
        applyTransform(element);
    });

    hammerManager.on('panstart', (e) => {
        // Prevent pan if drawing
        if (isDrawing) {
            // hammerManager.stop(true); // Stop Hammer from processing pan if drawing starts mid-pan
            return;
        }
        // Prevent 1-finger pan if zoomed in
        if (currentScale > 1 && e.pointers.length < 2) {
             // hammerManager.stop(true);
             return;
         }
        // If not zoomed, and it's a 1-finger vertical drag, let the browser handle scrolling
         if (currentScale === 1 && e.pointers.length === 1 && Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
             // Let default scroll happen, don't start Hammer pan
             return;
         }

        // Store initial offsets for valid pans (zoomed 2-finger, or unzoomed horizontal 1-finger - though horizontal is less likely needed)
    ...
    hammerManager.on('panmove', (e) => {
        ...
        // Ensure 2 fingers if zoomed
        if (currentScale > 1 && e.pointers.length < 2) return;

        // If not zoomed, ignore vertical pan (allow scroll) and horizontal 1-finger pan
         if (currentScale === 1) {
             if (e.pointers.length === 1 && Math.abs(e.deltaY) > Math.abs(e.deltaX)) return; // Allow scroll
             if (e.pointers.length === 1 && Math.abs(e.deltaX) > Math.abs(e.deltaY)) return; // Prevent horizontal 1-finger pan when not zoomed
             // Allow 2-finger pan even if not zoomed? (Could be confusing) - currently allows
         }
        ...
        // Calculate limits and constrain
        adjustPanLimits(element); // Recalculate limits just in case
        const rect = element.getBoundingClientRect(); // Get current size after potential scaling
    ...
    hammerManager.on('panend pancancel', (e) => {
        ...
        // Update initial offsets after pan ends
        initialOffsetX = currentOffsetX;
        initialOffsetY = currentOffsetY;
    });
}
...
function adjustPanLimits(element) {
    if (!element || currentScale <= 1) {
        currentOffsetX = 0; currentOffsetY = 0; return;
    }
    // Calculate new limits based on current scale
    const rect = element.getBoundingClientRect();
    const parentRect = flipbook.getBoundingClientRect();
    const maxMoveX = Math.max(0, (element.offsetWidth * currentScale - parentRect.width) / 2); // Use offsetWidth * scale for potential size
...
function getDrawPosition(e, canvas) {
    ...
    const pageWrap = canvas.closest('.page-wrap');
    if (!pageWrap) return { x: 0, y: 0 };
    const canvasRect = canvas.getBoundingClientRect();
    const scaleXCanvas = canvas.width / canvasRect.width;
    const scaleYCanvas = canvas.height / canvasRect.height;
    let clientX, clientY;
    if (e.touches && e.touches.length > 0) { clientX = e.touches[0].pageX - window.scrollX; clientY = e.touches[0].pageY - window.scrollY; }
    else if (e.changedTouches && e.changedTouches.length > 0) { clientX = e.changedTouches[0].pageX - window.scrollX; clientY = e.changedTouches[0].pageY - window.scrollY; }
    else { clientX = e.clientX; clientY = e.clientY; }
    const screenX = clientX - canvasRect.left;
    const screenY = clientY - canvasRect.top;
    return { x: screenX * scaleXCanvas, y: screenY * scaleYCanvas };
}
...
function loadHighlights(pageNumber) {
    ...
    if (dataUrl) {
        ...
        img.onerror = () => { console.error("Failed load highlight image for page", pageNumber); localStorage.removeItem(`flipbook-highlights-page-${pageNumber}`); }; // Fixed: Added braces
        img.src = dataUrl;
    } // Fixed: Added braces
} // Fixed: Added braces
