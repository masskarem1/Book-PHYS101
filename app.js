...
if(aiHelperToggle)aiHelperToggle.addEventListener("click",()=>{const e=getCurrentChapter();if(aiChapterTitleEl)aiChapterTitleEl.textContent=e?e.title:"Current Page";if(aiResponseEl)aiResponseEl.innerHTML=""; if(translateAnalysisBtn) translateAnalysisBtn.style.display = 'none'; /* Hide translate analysis btn */ if(aiModal)aiModal.style.display="flex"});
...
async function callTranslationAPI(textToTranslate) {
    ...
    try {
        ...
        return { success: false, message: `Translation Error: ${err.message}` };
    }
}
...
async function translateCurrentPage() {
    ...
    if (result.success) {
        ...
        overlay.textContent = result.message; // Show error message
    }
}
